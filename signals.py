...
def compute_scalp_signal(
    ...
    *,
    ...
    # VWAP proximity used for PRE alerts (so entry is ABOVE last for longs / BELOW last for shorts)
    vwap_prox = None
    try:
        vwap_prox = max(0.15 * float(atr_last or 0.0), 0.0008 * float(last_price or 0.0))
    except Exception:
        vwap_prox = None
    vwap_near = False
    if isinstance(ref_vwap, (float, int)) and isinstance(last_price, (float, int)) and isinstance(vwap_prox, (float, int)) and vwap_prox > 0:
        vwap_near = bool(abs(float(last_price) - float(ref_vwap)) <= float(vwap_prox))
    ...
    # PRE condition: near VWAP on the "wrong" side, with momentum/structure pointing toward a flip.
    pre_long_ok = bool(
    ...
    # Primary trigger must exist (otherwise we have nothing to anchor an entry plan).
    ...
    if confirmation_score < confirm_threshold:
        if pre_long_ok or pre_short_ok:
            stage = "PRE"
            stage_note = f"PRE: forming setup near VWAP (confirmations {confirmation_score}/{confirm_threshold})"
        else:
            return SignalResult(
                symbol, "NEUTRAL", _cap_score(max(long_points, short_points)),
